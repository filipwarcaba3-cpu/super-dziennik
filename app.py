#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Super dziennik 4.13.1 - poprawiony Dział IT, historia zmian i blokowanie kont."""
...
def layout(title,body,u=None,path=''):
    ...
    return f"<!doctype html><html lang='pl'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta name='theme-color' content='#111827'><title>{esc(title)} · {esc(name)}</title><style>{CSS}</style></head><body class='{theme_class}{it_class}'><div class='app'><aside class='side'><div class='logo'>{esc(name)} <span>4.13.1</span></div><div class='who'><b>{esc(u['full_name'])}</b><small>{role}</small></div>{school_switch}<nav class='nav'>{nav_items(u['role'],path)}</nav><div class='nav logout'><a href='/profile' aria-label='Profil'><span>◉</span><span class='nav-label'>Profil</span></a></div></aside><main class='main'><header class='top'><h2>{esc(title)}</h2><div style='display:flex;align-items:center;gap:14px'><span class='muted'>{'Dział IT' if is_it(u) else 'Szkoła'}</span><a class='btn red' href='/logout' aria-label='Wyloguj się'>↪ Wyloguj się</a></div></header><nav class='mobile-nav'>{mobile_nav}<a href='/profile'><span>◉</span><span class='nav-label'>Profil</span></a></nav><section class='content'>{body}</section></main></div></body></html>"
...
def it_users_page(u,c):
    ...
    students=q(c,"SELECT us.id user_id,st.id student_id,us.full_name,us.username,cl.name class_name,st.class_id FROM students st JOIN users us ON us.id=st.user_id LEFT JOIN classes cl ON cl.id=st.class_id WHERE us.school_id=? ORDER BY cl.name,us.full_name",(u['school_id'],))
    classes=q(c,'SELECT id,name FROM classes WHERE school_id=? ORDER BY name',(u['school_id'],))
    ...
    for x in students:
        ...
        sr.append("<tr><td><b>%s</b>%s</td><td>%s</td><td>%s</td></tr>"%(esc(x['full_name']),name_form,esc(x['class_name'] or '—'),cred_form(x['user_id'],x['username'])))
    sr=''.join(sr) or '<tr><td colspan=3>Brak uczniów.</td></tr>'
    ...
    body="<div class=hero><div><h1>Użytkownicy szkoły</h1><div class=muted>Dział IT może dodawać uczniów i nauczycieli oraz zmieniać dane logowania w edytowanej szkole.</div></div></div><div class=grid>"+add_teacher+add_student+"<div class='card col6'><h3>Nauczyciele</h3><div class=table-scroll><table class=table><tr><th>Nauczyciel</th><th>Login / hasło</th></tr>"+tr+"</table></div></div><div class='card col6'><h3>Uczniowie</h3><div class=table-scroll><table class=table><tr><th>Uczeń / imię i nazwisko</th><th>Klasa</th><th>Login / hasło</th></tr>"+sr+"</table></div></div></div>"
...
if __name__=='__main__':
    init(); print(f'Super dziennik 4.13: http://{HOST}:{PORT}'); ThreadingHTTPServer((HOST,PORT),Handler).serve_forever()
