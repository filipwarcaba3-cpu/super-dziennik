...
def lesson_gradebook(u,c,l,day):
    ...
    add=f"<form class=form method=post action='/lesson/{l['id']}/grade-column'><input type=hidden name=date value='{day.isoformat()}'><label><b>1. Rodzaj oceny</b><select name=category required><option>Sprawdzian</option><option>Kartkówka</option><option>Odpowiedź ustna</option><option>Praca domowa</option><option>Aktywność</option><option>Projekt</option><option>Inne</option></select></label><label><b>2. Nazwa</b><input name=name placeholder='Np. Ułamki zwykłe' required></label><div class=row><label>Waga<input name=weight type=number step=.5 min=.5 value='1'></label><label>Kolor<select name=color>{grade_color_options('black')}</select></label></div><button class='btn grade-col-add' title='Dodaj kolumnę'>+</button></form>"
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=='/profile/password':
                ...
                c.execute('INSERT INTO grade_columns(school_id,class_id,subject_id,lesson_id,category,name,weight,color,created_by) VALUES(?,?,?,?,?,?,?,?,?)',(u['school_id'],l['class_id'],l['subject_id'],lid,data.get('category','Inne').strip(),data['name'].strip(),float(data.get('weight','1') or 1),color,u['id']))
