#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Super dziennik 4.12 - Dział IT, zarządzanie szkołami i wsparcie techniczne."""
...
def nav_items(role,path):
    if role in ('it_admin','it_staff'):
        ...
        if role=='it_admin':
            base.insert(1,('/it/schools','Szkoły','◆')); base += [('/it/staff','Pracownicy IT','⚙'),('/it/journal','Informacje o dzienniku','A')]
    else:
...
def it_dashboard(u,c):
    ...
    body=f"<div class=hero><div><h1>Panel Działu IT</h1><div class=muted>Obsługa techniczna szkoły: {esc(school['name'])}.</div></div></div><div class=stats><div class=stat><div class=label>Szkoła</div><div class=num>{esc(school['name'])}</div></div><div class=stat><div class=label>Nauczyciele</div><div class=num>{teachers}</div></div><div class=stat><div class=label>Uczniowie</div><div class=num>{students}</div></div><div class=stat><div class=label>Rola</div><div class=num>{'Admin IT' if u['role']=='it_admin' else 'IT'}</div></div></div><div class='grid' style='margin-top:16px'><div class='card col6'><h3>Szybkie działania</h3><p><a class=btn href='/it/users'>Użytkownicy szkoły</a></p><p><a class='btn gray' href='/attendance'>Frekwencja</a></p><p><a class='btn gray' href='/it/school-info'>Informacje szkoły</a></p></div><div class='card col6'><h3>Uprawnienia akademickie</h3><p>Możesz przeglądać oceny, uwagi i pochwały w kartach uczniów oraz zmieniać frekwencję.</p><p class=muted>Dział IT nie może wystawiać ani edytować ocen, uwag i pochwał.</p></div></div>"
...
def it_staff_page(u,c):
    ...
    rows=''.join(f"<tr><td><b>{esc(x['full_name'])}</b><br><small>{esc(x['username'])}</small></td><td>{esc(x['school_name'] or '—')}</td><td>{esc(x['email'] or '—')}</td><td><form method=post action='/it/staff/{x['id']}/delete' onsubmit='return confirm(&quot;Usunąć pracownika IT?&quot;)'><button class='btn sm red'>Usuń</button></form></td></tr>" for x in staff) or '<tr><td colspan=4>Brak pracowników IT.</td></tr>'
