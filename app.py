...
CREATE TABLE IF NOT EXISTS grade_columns(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,school_id INTEGER NOT NULL,class_id INTEGER NOT NULL,subject_id INTEGER NOT NULL,lesson_id INTEGER,name TEXT NOT NULL,weight REAL DEFAULT 1,color TEXT DEFAULT 'black',created_by INTEGER NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(class_id) REFERENCES classes(id) ON DELETE CASCADE,FOREIGN KEY(subject_id) REFERENCES subjects(id),FOREIGN KEY(lesson_id) REFERENCES lessons(id) ON DELETE CASCADE,FOREIGN KEY(created_by) REFERENCES users(id));
...
def lesson_gradebook(u,c,l,day):
    ...
    head="<th class=student-name>Uczeń</th>"+''.join(f"<th>{esc(x['name'])}<br><small>waga {esc(x['weight'])}</small></th>" for x in cols)
    bodyrows=''
    for st in students:
        cells=''
        for col in cols:
    ...
    add=f"<form class=form method=post action='/lesson/{l['id']}/grade-column'><input type=hidden name=date value='{day.isoformat()}'><div class=row><input name=name placeholder='Nazwa kolumny, np. Kartkówka 1' required><input name=weight type=number step=.5 min=.5 value='1' placeholder='Waga'></div><select name=color>{grade_color_options('black')}</select><button class='btn grade-col-add' title='Dodaj kolumnę'>+</button></form>"
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=='/profile/password':
                ...
                c.execute('INSERT INTO grade_columns(school_id,class_id,subject_id,lesson_id,name,weight,color,created_by) VALUES(?,?,?,?,?,?,?,?)',(u['school_id'],l['class_id'],l['subject_id'],lid,data['name'].strip(),float(data.get('weight','1') or 1),color,u['id']))
                ...
                for st in students:
                    for col in cols:
                        ...
                        elif val: c.execute('INSERT INTO grades(student_id,subject_id,teacher_id,value,weight,category,comment,lesson_id,date,color,column_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)',(st['id'],l['subject_id'],u['id'],val,col['weight'],col['name'],'',lid,data.get('date') or date.today().isoformat(),col['color'],col['id']))
