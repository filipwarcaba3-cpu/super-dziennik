...
def student_page(u,c,sid):
    ...
    manage_forms=(f"<div class='card col12'><h3>Zarządzanie uczniem</h3><div class=row>{("<a class='btn' href='/grades?student_id=%s'>Dodaj ocenę</a>"%sid) if can_academic else ''}<a class='btn gray' href='/attendance?student_id={sid}'>Dodaj / popraw frekwencję</a></div></div>" if can_attendance else '')
...
def grades_page(u,c):
    ...
    if u['role']=='student':
        ...
        form="<div class='card col12'><h3>Dodaj ocenę</h3><form class=form method=post action=/grade><div class=row><select name=student_id required>"+opts(students,'id','full_name')+"</select><select name=subject_id required>"+opts(subjects,'id','name')+"</select></div><div class=row><input name=value placeholder='Ocena, np. 5' required><input name=weight type=number step=.5 value=1 min=.5></div><div class=row><input name=category value='Ocena'><input name=comment placeholder='Komentarz'></div><div class=row><select name=color>"+grade_color_options('black')+"</select><div class=muted>Kolor oceny widoczny dla ucznia</div></div><button class=btn>Dodaj ocenę</button></form></div>"
    bysub=''
    if u['role']=='student':
