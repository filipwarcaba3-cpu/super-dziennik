...
def lesson_detail_for_date(u,c,l,day):
    ...
    for idx,st in enumerate(students,1):
        ...
        rows += f"<div class=attendance-student><b>{idx}. {esc(st['full_name'])}</b><div class=attendance-buttons>{basic}<label onclick=\"let e=document.getElementById('extra_{st['id']}');e.style.display=e.style.display==='none'?'block':'none'\"><span class=attendance-more>inne</span></label></div>{extra}</div>"
