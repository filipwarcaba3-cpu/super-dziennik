...
def lesson_card(x,u,c,day=None):
    ...
    planned=one(c,'SELECT type,name FROM tests WHERE class_id=? AND subject_id=? AND date=? AND (target_lesson_id=? OR target_lesson_id IS NULL) ORDER BY id LIMIT 1',(x['class_id'],x['subject_id'],day.isoformat(),x['id']))
