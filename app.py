#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Super dziennik 4.11 - lekcje grupowe/indywidualne i opcja bez powtarzania."""
...
def messages_page(u,c,reply_id=None):
    ...
    reply=None
    if reply_id:
        reply=one(c,'SELECT m.*,s.full_name sender FROM messages m JOIN users s ON s.id=m.sender_id WHERE m.id=? AND m.recipient_id=? AND m.school_id=?',(reply_id,u['id'],u['school_id']))
    selected_recipient=row_get(reply,'sender_id') if reply else None
    subject=''
    if reply:
        original=str(reply['subject'] or '').strip()
        subject=original if original.upper().startswith('RE:') else 'RE: '+original
    heading='Odpowiedz na wiadomość' if reply else 'Nowa wiadomość'
    reply_hint=f"<div class=notice>Odpowiadasz na wiadomość od <b>{esc(reply['sender'])}</b>.</div>" if reply else ''
    form="<div class='card col4'><h3>"+heading+"</h3>"+reply_hint+"<form class=form method=post action=/message><select name=recipient_id>"+opts(users,'id','full_name',selected_recipient)+"</select><input name=subject placeholder='Temat' value='"+esc(subject)+"' required><textarea name=body placeholder='Treść' required></textarea><button class=btn>Wyślij</button></form></div>"
    inboxhtml=''.join("<article style='padding:12px 0;border-bottom:1px solid var(--line)'><b>%s</b><br><small>od %s · %s</small><p>%s</p><a class='btn sm gray' href='/messages?reply=%s'>Odpowiedz</a></article>"%(esc(x['subject']),esc(x['sender']),esc(x['created_at']),esc(x['body']),x['id']) for x in inbox) or '<div class=empty>Brak wiadomości.</div>'
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=='/profile/password':
                ...
                rid=int(data['recipient_id']); recipient=one(c,'SELECT id FROM users WHERE id=? AND school_id=? AND active=1',(rid,u['school_id']))
...
if __name__=='__main__':
    init(); print(f'Super dziennik 4.11: http://{HOST}:{PORT}'); ThreadingHTTPServer((HOST,PORT),Handler).serve_forever()
